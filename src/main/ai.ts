import type { ActivityRecord, VisionResult } from '../shared/types';
...
export async function generateReport(
  ...
  const { visionResults, records, template, reportType, startDate, endDate } = params;
...
3. 明天计划什么`;
  ...
  if (!visionText && !recordsText) {
    userContent += '（无工作记录数据，请生成空报告模板）';
  }

  // Log only request counts. Do not write prompt content to disk because it can
  // contain private screen-recognition summaries and report material.
  console.log('\n[生成报告] model:', model, '| vision:', visionResults.length, '条 | records:', records.length, '条');
